...
const NewClientForm: React.FC<{onCancel: () => void}> = ({onCancel}) => {
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    const savedClients = JSON.parse(localStorage.getItem('finpay_clients') || '[]');
    localStorage.setItem('finpay_clients', JSON.stringify([newClient, ...savedClients]));
          ...
          <form onSubmit={handleSubmit} className="bg-white dark:bg-slate-900 rounded-[2.5rem] shadow-xl border border-slate-200 dark:border-white/5 overflow-hidden">
             ...
             <div className="p-8 bg-slate-100 dark:bg-black/40 flex flex-col md:flex-row items-center justify-between gap-8 border-t border-slate-200 dark:border-white/5">
                <div className="flex items-center gap-4 text-slate-500 dark:text-slate-400">
                   ...
                   <div>
                      <p className="text-[10px] font-black uppercase tracking-widest leading-none mb-1">Segurança de Dados</p>
                      <p className="text-[11px] font-medium opacity-60">Os dados informados são armazenados localmente no seu navegador.</p>
                   </div>
                </div>
