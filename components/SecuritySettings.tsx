...
const SecuritySettings: React.FC = () => {
  ...
  const exportBackup = () => {
    ...
    try {
      ...
      link.href = url;
      link.download = `finpay-backup-${new Date().toISOString().split('T')[0]}.json`;
  ...
  const handleImport = (event: React.ChangeEvent<HTMLInputElement>) => {
    ...
    reader.onload = (e) => {
      try {
        ...
        // Validação básica
        if (!data.version && !data.followups) {
        ...
        setMessage({type: 'success', text: 'Dados restaurados! Reiniciando...'});
